...
$choices = [xl('CSV File'), xl('Email'), xl('Phone call list')];
$gender = [xl('Any'), xl('Male'), xl('Female')];
$hipaa = [xl('NO'), xl('YES')];
$sort_by = [xl('Zip Code') => 'patient_data.postal_code', xl('Last Name') => 'patient_data.lname', xl('Appointment Date') => 'last_ap'];

// process form
if ($_POST['form_action'] == 'Process') {
    $_POST['app_s']          = prepareDateBeforeSave($_POST['app_s']);
    $_POST['app_e']          = prepareDateBeforeSave($_POST['app_e']);
    $_POST['seen_since']     = prepareDateBeforeSave($_POST['seen_since']);
    $_POST['not_seen_since'] = prepareDateBeforeSave($_POST['not_seen_since']);
    ...
    if (!check_select($_POST['hipaa_choice'], $hipaa)) {
        $form_err .= xl('Error in "HIPAA" selection', '', '<br>');
    }
    if (!check_select($_POST['sort_by'], $sort_by)) {
        $form_err .= xl('Error in "Sort By" selection', '', '<br>');
    }
    // validates and or
    if (!check_yes_no($_POST['and_or_gender'])) {
        $form_err .= xl('Error in YES or NO option', '', '<br>');
    }
    if (!check_yes_no($_POST['and_or_app_within'])) {
        $form_err .= xl('Error in YES or NO option', '', '<br>');
    }
    if (!check_yes_no($_POST['and_or_seen_since'])) {
        $form_err .= xl('Error in YES or NO option', '', '<br>');
    }
    if (!check_yes_no($_POST['and_or_not_seen_since'])) {
        $form_err .= xl('Error in YES or NO option', '', '<br>');
    }
            ...
            <table class="table">
                ...
                <tr>
                    <td>
                        ...
                        <?php xl('Overwrite HIPAA choice', 'e') ?> :
                    </td>
                    <td align='left'><SELECT NAME="hipaa_choice">
                            <?php
                ...
                <tr>
                    <td>
                        <?php xl('Age From', 'e') ?>:<INPUT TYPE="text" size="2" NAME="age_from"></td>
                    <td> <?php xl('Up to', 'e') ?>:<INPUT TYPE="text" size="2" NAME="age_upto"></td>
                    <td>
                        <?php xl('And', 'e') ?>:<INPUT TYPE="radio" NAME="and_or_gender" value="AND" checked>, <?php xl('Or', 'e') ?>:<INPUT
                            TYPE="radio" NAME="and_or_gender" value="OR"></td>
                    <td>
                ...
                <tr>
                    <td colspan='3'>
                        (<?php xl('Fill here only if sending email notification to patients', 'e') ?>)
                    </td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                ...
                <tr>
                    ...
                    <td><INPUT TYPE="text" NAME="email_sender" value="your@example.com"></td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
                    ...
                    <td><INPUT TYPE="text" NAME="email_subject" value="From your clinic"></td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>
                        ...
                        <INPUT TYPE="submit" name="form_action" value=<?php xl('Process', 'e', '\'', '\''); ?>></td>
                    <td><?php xl('Process takes some time', 'e') ?></td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                </tr>
            </table>
